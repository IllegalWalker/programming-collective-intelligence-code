...
def _int_array(seq):
    size = len(seq)
    array = svm.new_int(size)
    i = 0
    for item in seq:
        svm.int_setitem(array, i, item)
        i = i + 1
    return array
...
def _double_array(seq):
    size = len(seq)
    array = svm.new_double(size)
    i = 0
    for item in seq:
        svm.double_setitem(array, i, item)
        i = i + 1
    return array
...
def _int_array_to_list(x, n):
    return map(svm.int_getitem, [x] * n, range(n))


def _double_array_to_list(x, n):
    return map(svm.double_getitem, [x] * n, range(n))
...
class svm_parameter:
    ...
    def __init__(self, **kw):
        self.__dict__['param'] = svm.new_svm_parameter()
    ...
    def __getattr__(self, attr):
        get_func = getattr(svm, 'svm_parameter_%s_get' % (attr))
    ...
    def __setattr__(self, attr, val):
        ...
        set_func = getattr(svm, 'svm_parameter_%s_set' % (attr))
        set_func(self.param, val)

    def __repr__(self):
        ret = '<svm_parameter:'
        for name in dir(svm):
            if name[:len('svm_parameter_')] == 'svm_parameter_' and name[-len(
                    '_set'):] == '_set':
                attr = name[len('svm_parameter_'):-len('_set')]
                if attr == 'weight_label':
                    ret = ret + ' weight_label = %s,' % _int_array_to_list(
                        self.weight_label, self.weight_label_len)
                elif attr == 'weight':
                    ret = ret + ' weight = %s,' % _double_array_to_list(
                        self.weight, self.weight_len)
                else:
    ...
    def __del__(self):
        _free_int_array(self.weight_label)
        _free_double_array(self.weight)
        svm.delete_svm_parameter(self.param)
...
def _convert_to_svm_node_array(x):
    ...
    if type(x) == dict:
        for k, v in x.iteritems():
            # all zeros kept due to the precomputed kernel; no good solution
            #  yet
            #			if v != 0:
            iter_range.append(k)
    ...
    iter_range.sort()
    data = svm.svm_node_array(len(iter_range) + 1)
    svm.svm_node_array_set(data, len(iter_range), -1, 0)

    j = 0
    for k in iter_range:
        svm.svm_node_array_set(data, j, k, x[k])
        j = j + 1
    return data
...
class svm_problem:
    def __init__(self, y, x):
        assert len(y) == len(x)
        self.prob = prob = svm.new_svm_problem()
        self.size = size = len(y)

        self.y_array = y_array = svm.new_double(size)
        for i in range(size):
            svm.double_setitem(y_array, i, y[i])

        self.x_matrix = x_matrix = svm.svm_node_matrix(size)
        self.data = []
        self.maxlen = 0;
        for i in range(size):
            data = _convert_to_svm_node_array(x[i])
            self.data.append(data);
            svm.svm_node_matrix_set(x_matrix, i, data)
            if type(x[i]) == dict:
                if (len(x[i]) > 0):
        ...
        svm.svm_problem_l_set(prob, size)
        svm.svm_problem_y_set(prob, y_array)
        svm.svm_problem_x_set(prob, x_matrix)

    def __repr__(self):
        return "<svm_problem: size = %s>" % (self.size)

    def __del__(self):
        svm.delete_svm_problem(self.prob)
        svm.delete_double(self.y_array)
        for i in range(self.size):
            svm.svm_node_array_destroy(self.data[i])
        svm.svm_node_matrix_destroy(self.x_matrix)
...
class svm_model:
    def __init__(self, arg1, arg2=None):
        if arg2 == None:
            ...
            filename = arg1
            self.model = svm.svm_load_model(filename)
        else:
            ...
            msg = svm.svm_check_parameter(prob.prob, param.param)
            if msg: raise ValueError, msg
            self.model = svm.svm_train(prob.prob, param.param)

        # setup some classwide variables
        self.nr_class = svm.svm_get_nr_class(self.model)
        self.svm_type = svm.svm_get_svm_type(self.model)
        # create labels(classes)
        intarr = svm.new_int(self.nr_class)
        svm.svm_get_labels(self.model, intarr)
        self.labels = _int_array_to_list(intarr, self.nr_class)
        svm.delete_int(intarr)
        # check if valid probability model
        self.probability = svm.svm_check_probability_model(self.model)

    def predict(self, x):
        data = _convert_to_svm_node_array(x)
        ret = svm.svm_predict(self.model, data)
        svm.svm_node_array_destroy(data)
        return ret
    ...
    def predict_values_raw(self, x):
        ...
        data = _convert_to_svm_node_array(x)
        dblarr = svm.new_double(n)
        svm.svm_predict_values(self.model, data, dblarr)
        ret = _double_array_to_list(dblarr, n)
        svm.delete_double(dblarr)
        svm.svm_node_array_destroy(data)
        return ret
    ...
    def predict_probability(self, x):
        ...
        if self.svm_type == NU_SVR or self.svm_type == EPSILON_SVR:
            raise TypeError, "call get_svr_probability or get_svr_pdf for " \
                             "probability output of regression"
        ...
        # convert x into svm_node, alloc a double array to receive
        # probabilities
        data = _convert_to_svm_node_array(x)
        dblarr = svm.new_double(self.nr_class)
        pred = svm.svm_predict_probability(self.model, data, dblarr)
    ...
    def get_svr_probability(self):
        ...
        ret = svm.svm_get_svr_probability(self.model)
        if ret == 0:
    ...
    def save(self, filename):
        svm.svm_save_model(filename, self.model)

    def __del__(self):
        svm.svm_destroy_model(self.model)


def cross_validation(prob, param, fold):
    if param.gamma == 0:
        param.gamma = 1.0 / prob.maxlen
    dblarr = svm.new_double(prob.size)
    svm.svm_cross_validation(prob.prob, param.param, fold, dblarr)
